...
const inputClass =
  'w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-emerald-500'
          ...
          <table className="min-w-full divide-y divide-gray-100 text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                rows.map((r) => (
                  <tr className="hover:bg-gray-50/80" key={r.id}>
                    <td className="px-4 py-3 font-mono text-sm font-medium text-gray-900">{r.name}</td>
                    <td className="max-w-md px-4 py-3 text-gray-600">{r.description ?? '—'}</td>
                    <td className="px-4 py-3 tabular-nums text-gray-800">{r.permissionCodes?.length ?? 0}</td>
      ...
      {modalOpen && editing ? (
        ...
        >
          <button
            ...
            type="button"
          />
          <div className="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-gray-100 bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Sửa vai trò: {editing.name}</h2>
                <p className="mt-1 text-xs text-gray-500">Chọn mã permission từ catalog đã đồng bộ.</p>
              </div>
              <button
            ...
            <form className="space-y-4" onSubmit={handleSave}>
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700" htmlFor="role-desc">
                  Mô tả
                </label>
                <textarea
              ...
              <div>
                <div className="mb-2 text-sm font-medium text-gray-700">Permission (mã)</div>
                {permQuery.isError ? (
                  <p className="text-sm text-red-600">
                    {permQuery.error instanceof Error ? permQuery.error.message : 'Không tải catalog'}
                  </p>
                ) : (
                  <div className="max-h-56 space-y-4 overflow-y-auto rounded-xl border border-gray-100 bg-gray-50/50 p-3">
                    {groupedPerms.length === 0 ? (
                      <p className="text-sm text-gray-500">Catalog trống.</p>
                    ) : (
                      groupedPerms.map(([module, list]) => (
                        <div key={module}>
                          ...
                          <ul className="space-y-2">
                            {list.map((p) => (
                              <li key={p.id}>
                                <label className="flex cursor-pointer items-start gap-2 rounded-lg px-1 py-1 hover:bg-white">
                                  <input
