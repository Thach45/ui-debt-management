  ...
  return (
    <div className="mx-auto max-w-6xl px-5 text-left md:px-6">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-slate-100">Hợp đồng vay</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Quản lý các hợp đồng vay và trả góp</p>
        </div>
        <Link
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0">
            ...
            <button
              className="ml-0.5 flex shrink-0 items-center gap-1 rounded-lg border border-gray-200 px-2.5 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
              disabled={isFetching}
          ...
          <table className="w-full min-w-[820px] border-collapse text-left text-sm">
            <thead>
              <tr className="bg-gray-50 text-xs font-medium uppercase tracking-wide text-gray-500 dark:bg-slate-950/50 dark:text-slate-400">
                ...
                <th className="px-4 py-3 text-right">Gốc còn lại</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {isPending ? (
                <tr>
                  <td className="px-4 py-10 text-center text-sm text-gray-500 dark:text-slate-400" colSpan={7}>
                    Đang tải…
                  </td>
                </tr>
              ) : isError ? (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Link
                        ...
                        to={`/contracts/${row.id}`}
                      >
                        <div className="text-base font-medium text-gray-900 transition-colors group-hover:text-emerald-600 dark:text-slate-100 dark:group-hover:text-emerald-200">
                          {row.customerName}
                        </div>
                        <div className="text-sm text-gray-500 dark:text-slate-400">{row.customerPhone}</div>
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right text-base font-medium tabular-nums text-emerald-600">
                      {formatVnd(row.remainingPrincipal ?? row.principal)}
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-sm leading-snug text-gray-500 dark:text-slate-400">
        ...
        {data && data.totalPages > 1 ? (
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-gray-100 px-5 py-3.5 text-sm text-gray-600 dark:border-slate-800 dark:text-slate-400">
            <span>
              Trang {data.number + 1} / {data.totalPages} — {data.totalElements} hợp đồng
            </span>
            <div className="flex gap-2">
              <button
                className="rounded-lg border border-gray-200 px-4 py-2 font-medium text-gray-800 transition hover:bg-gray-50 disabled:opacity-40 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                disabled={data.first}
