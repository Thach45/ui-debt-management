  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-6 text-left md:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <Link
          className="flex items-center gap-2 text-sm text-gray-500 transition-colors hover:text-gray-900 dark:text-slate-400 dark:hover:text-slate-100"
          to="/customers"
        >
        ...
        <Link
          className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-emerald-700 shadow-sm hover:bg-emerald-50 dark:border-slate-700 dark:bg-slate-900/60 dark:text-emerald-200 dark:hover:bg-emerald-950/40"
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
            <div className="mb-6 flex flex-col items-center text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100">{customer.name}</h2>
              <span className="mt-2">
                <CustomerTierBadge tier={customer.customerTier} />
              </span>
            </div>

            <div className="space-y-4 border-t border-gray-100 pt-6 dark:border-slate-800">
              <div className="flex items-start gap-3 text-sm">
                <Phone className="mt-0.5 size-4 shrink-0 text-gray-400 dark:text-slate-500" aria-hidden />
                <span className="text-gray-900 dark:text-slate-100">{customer.phone}</span>
              </div>
              <div className="flex items-start gap-3 text-sm">
                <MapPin className="mt-0.5 size-4 shrink-0 text-gray-400 dark:text-slate-500" aria-hidden />
                <div>
                  <span className="mb-0.5 block text-xs text-gray-500 dark:text-slate-400">Địa chỉ</span>
                  <span className="text-gray-900 dark:text-slate-100">{customer.address}</span>
                </div>
              </div>
              <div className="flex items-start gap-3 text-sm">
                <MapPin className="mt-0.5 size-4 shrink-0 text-emerald-500" aria-hidden />
                <div>
                  <span className="mb-0.5 block text-xs text-gray-500 dark:text-slate-400">Vị trí rẫy / vườn</span>
                  <span className="font-medium text-gray-900 dark:text-slate-100">{customer.farmingLocation ?? '—'}</span>
                </div>
              </div>
              <div className="flex items-start gap-3 text-sm">
                <Calendar className="mt-0.5 size-4 shrink-0 text-gray-400 dark:text-slate-500" aria-hidden />
                <div>
                  <span className="mb-0.5 block text-xs text-gray-500 dark:text-slate-400">Ngày tạo</span>
                  <span className="text-gray-900 dark:text-slate-100">{formatDate(customer.createdAt)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-orange-100 bg-orange-50 p-6 shadow-sm dark:border-orange-900/40 dark:bg-orange-950/15">
        ...
        <div className="lg:col-span-2">
          <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
            <div className="border-b border-gray-100 p-6 dark:border-slate-800">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-slate-100">Danh sách hợp đồng</h3>
            </div>
            {contracts.length === 0 ? (
                ...
                {contracts.map((c) => {
                  ...
                  return (
                    <button
                      key={c.id}
                      type="button"
                      className="flex w-full flex-col gap-4 p-6 text-left transition-colors hover:bg-gray-50 sm:flex-row sm:items-center sm:justify-between dark:hover:bg-slate-950/40"
                      onClick={() => navigate(`/contracts/${c.id}`)}
                    >
                      <div>
                        <h4 className="mb-1 font-medium text-gray-900 dark:text-slate-100">{title}</h4>
                      ...
                      <div className="text-left sm:text-right">
                        ...
                        <div className="text-xs text-gray-500 dark:text-slate-400">Còn lại</div>
                      </div>
                    </button>
                  )
                })}
