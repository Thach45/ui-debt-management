    ...
    <div className="mx-auto max-w-6xl px-5 text-left md:px-6">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="mb-1 text-sm font-medium text-gray-500">Tổng khách</div>
          <div className="text-xl font-bold tabular-nums text-gray-900">{rows.length}</div>
        </div>
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="mb-1 text-sm font-medium text-gray-500">Đang hiển thị (lọc)</div>
          <div className="text-xl font-bold tabular-nums text-emerald-600">{filtered.length}</div>
        </div>
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="mb-1 text-sm font-medium text-gray-500">Tổng dư nợ (danh sách)</div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-gray-100 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            className="flex shrink-0 items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50"
            disabled={isFetching}
              ...
              {isPending ? (
                ...
                filtered.map((row) => (
                  <tr className="hover:bg-gray-50/80" key={row.id}>
                    ...
                    <td className="px-4 py-3 tabular-nums text-gray-800">{row.phone}</td>
                    <td className="max-w-[200px] truncate px-4 py-3 text-gray-600" title={row.address ?? ''}>
                      {row.address ?? '—'}
                    </td>
                    <td className="px-4 py-3">
